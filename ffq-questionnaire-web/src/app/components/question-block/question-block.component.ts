...
export class QuestionBlockComponent implements OnChanges, OnInit {
  ...
  getImageRef(): void {
    switch (this.foodItem.name) {
      ...
      case 'Baby Cereal added to formula or breastmilk (examples: rice, wheat, etc.)':
        this.imageRef = '../assets/serving-size-images/Cereal-added-to-milk.jpg';
        break;
      ...
      case 'Flavored Milk (chocolate, vanilla or fruit flavored); examples: Nido, frozen beverages, milkshakes, hot chocolate, etc.)':
        this.imageRef = '../assets/serving-size-images/Flavored-Milk-tsp-oz .jpg';
        break;
  ...
  getHasPictures(): boolean {
    ...
    else if (this.foodItem.name === 'Other milks (example: soy, rice, almond, goat, etc.)') {
      return false;
    }
    ...
    else if (this.foodItem.name === 'Soy products: tofu, veggie Burger, edamame, soy nuggets') {
      return false;
    }
    else if (this.foodItem.name === '100% fruit juice (examples: apple, pear, grape, cranberry, etc.)') {
      return false;
    }
    else if (this.foodItem.name === 'Pizza, turnovers, tacos, other fried foods, etc.') {
      return false;
    }
    ...
    else if (this.foodItem.name === 'Candies (chocolate, gummies, M&Ms, lollipop, etc.)') {
      return false;
    }
    else if (this.foodItem.name === 'Chips (examples: Doritos, Lays, Cheetos, Tostitos, etc.)') {
      return false;
    }
  ...
  zoomPictures(): void {
    ...
    if (this.foodItem.name === 'Baby Cereal added to formula or breastmilk (examples: rice, wheat, etc.)') {
      data = {
        data: {
          title: "Baby Cereal added to formula or breastmilk (examples: rice, wheat, etc.)",
          imageUrl: this.imageRef,
          messages: [
            "This image represents the baby cereal.",
          ],
        },
      };
    }
    ...
    if (this.foodItem.name === 'Flavored Milk (chocolate, vanilla or fruit flavored); examples: Nido, frozen beverages, milkshakes, hot chocolate, etc.)') {
      data = {
        data: {
          title: "Flavored Milk (chocolate, vanilla or fruit flavored); examples: Nido, frozen beverages, milkshakes, hot chocolate, etc.)",
          imageUrl: this.imageRef,
          messages: [
            "This image represents .",
          ],
    ...
    if (this.foodItem.name === 'Vitamin Enriched Pediatric Milk Beverages (examples: Pediasure, Pediasure growth, etc.)') {
      data = {
        data: {
          title: "Vitamin Enriched Pediatric Milk Beverages (examples: Pediasure, Pediasure growth, etc.)",
          imageUrl: this.imageRef,
          messages: [
            "This image represents .",
          ],
    ...
    if (this.foodItem.name === '100% vegetable juice (examples: carrot, tomato, etc.)') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Other fruit beverages (examples: Hi-C, Space gang, Sunny D, etc.)') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Sodas (examples: Coca-Cola, Pepsi, 7-Up, Fanta, Malta, etc.)') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Kool-Aid, Tang, Iced Tea (examples: Nestea, Lipton, etc.)') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Hot cereal (examples: rice cereal, cream of wheat, oatmeal, corn mush, etc.)') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Ready to eat breakfast cereals, low sugar (examples: Corn flakes, regular Cheerios, Life, Corn or Rice Chex, Grape nuts, etc.)') {
      data = {
        data: {
          title: "Ready to eat breakfast cereals, low sugar (examples: Corn flakes, regular Cheerios, Life, Corn or Rice Chex, Grape nuts, etc.)",
          imageUrl: this.imageRef,
          messages: [
            "This image represents .",
          ],
    ...
    if (this.foodItem.name === 'Bread (examples: sandwich bread, dinner roll, bagel, French bread, tortilla...)') {
      data = {
        data: {
          title: "Bread (examples: sandwich bread, dinner roll, bagel, French bread, tortilla...)",
          imageUrl: this.imageRef,
          messages: [
            "This image represents the ounces for infant formula in a bottle.",
          ],
    ...
    if (this.foodItem.name === 'Pancake, waffles, French toast, cinnamon rolls, etc.') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Crackers (examples: soda, Ritz, Cheez-It, Saltines, etc.)') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Citrus fruits (examples: orange, mandarin, etc.)') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Other vegetables (examples: tomato, lettuce, broccoli, etc.)') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Other starchy vegetables (yucca, yautia, celery, breadfruit)') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Black/Brown/White beans, pigeon beans, chickpeas, etc.') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Beef or pork (includes ground beef, steak, ribs, hot dogs, sausage, ham)') {
      data = {
        data: {
          title: "Beef or pork (includes ground beef, steak, ribs, hot dogs, sausage, ham)",
          imageUrl: this.imageRef,
          messages: [
            "This image represents .",
          ],
    ...
    if (this.foodItem.name === 'Chicken or turkey (includes breast, thighs, wings, ground)') {
      data = {
        data: {
    ...
    if (this.foodItem.name === 'Cookies (sugar cookies, chocolate chips, oats, vanilla, etc.)') {
      data = {
        data: {
          title: "Cookies (sugar cookies, chocolate chips, oats, vanilla, etc.)",
          imageUrl: this.imageRef,
          messages: [
            "This image represents .",
          ],
